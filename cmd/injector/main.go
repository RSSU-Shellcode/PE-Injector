...
func init() {
	...
	flag.BoolVar(&opts.UseRWXPage, "rwx", false, "set shellcode memory page protect to PAGE_EXECUTE_READWRITE")
	flag.BoolVar(&opts.NotSaveContext, "nsc", false, "not append instruction about save and restore context")
	flag.BoolVar(&opts.NotSaveLastError, "nsl", false, "not generate code about save and restore last error")
...
func main() {
	...
	fmt.Println("type:", ctx.Type)
	fmt.Println("size:", ctx.Size)
	fmt.Println("raw: ", ctx.Raw)
...
func analyzeImage() {
	...
	fmt.Println("==================Extra=================")
	fmt.Println("HasSignature:       ", info.HasSignature)
	fmt.Println("HasLoadConfig:      ", info.HasLoadConfig)
	fmt.Println("========================================")
}
