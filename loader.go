...
func (inj *Injector) processLoader(ctx *loaderCtx, loader string, payload []byte) (string, error) {
	...
	for _, sw := range []bool{
		...
		inj.opts.ForceCodeCaveNS,
		inj.opts.ForceExtendText,
		inj.opts.ForceExtendTextNS,
...
func (inj *Injector) useCodeCaveNSMode(ctx *loaderCtx, loader string, payload []byte) (string, error) {
	...
	// check is used to calculate loader size
	if inj.loaderSize == 0 {
		ctx.PayloadRVA = paddingPayloadRVA
	...
	section, err := inj.createSectionRO(inj.opts.SectionName, size)
	if err != nil {
		return "", err
	}
...
func (inj *Injector) insertGarbageInst() string {
	if inj.opts.NoGarbageInst || inj.ctx.Mode != ModeCreateText {
		return ""
	}
